...
**Desktop app (macOS/Windows/Linux)** — see [`desktop/`](desktop/) — a native window around the same browser-version app, built with [Tauri](https://tauri.app). Single-device/no-accounts by default like the browser version, but photo and manual/PDF uploads *do* work here, saved to real files on your disk instead of being disabled — and if you also run the self-hosted server, [NAS sync](desktop/README.md#nas-sync) can point this app at that server's `/data` folder (mounted as a network share) for real cross-device sync, no server changes needed. A macOS build is produced automatically by CI on every push to `main` (**Actions → Build desktop app**, download the `patchdoc-macos` artifact) — Windows and Linux only build on an actual release (see below), or build any of the three yourself with [`desktop/README.md`](desktop/README.md).
