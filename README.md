...
- **Community modules** — browse and import module definitions shared by other users, in every version (browser, desktop, self-hosted) — see [`community-modules/`](community-modules/)
...
**Desktop app (macOS/Windows/Linux)** — see [`desktop/`](desktop/) — a native window around the same browser-version app, built with [Tauri](https://tauri.app). Still single-device/no-accounts like the browser version, but photo and manual/PDF uploads *do* work here, saved to real files on your disk instead of being disabled. A macOS build is produced automatically by CI on every push to `main` (**Actions → Build macOS desktop app**, download the `patchdoc-macos` artifact) — for Windows/Linux, or to build it yourself, see [`desktop/README.md`](desktop/README.md). To publish a downloadable `.dmg` as a [GitHub Release](../../releases) instead of a CI artifact, either push a version tag (`git tag v1.0.0 && git push origin v1.0.0`) or, without touching git locally, go to **Actions → Build macOS desktop app → Run workflow** and fill in a version number — the workflow tags and releases it for you.

Since this build isn't signed with a paid Apple Developer certificate, macOS may refuse to open it from `/Applications` ("PATCH.doc is damaged and can't be opened"). The app isn't actually damaged — this is Gatekeeper blocking unsigned software; the `.dmg` includes a README with the one-line fix (`xattr -cr /Applications/PATCH.doc.app`), also covered in [`desktop/README.md`](desktop/README.md#troubleshooting-macos-says-the-app-is-damaged).
